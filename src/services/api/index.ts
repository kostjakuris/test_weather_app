...
export const getCoordinates = createAsyncThunk('getCityCoordinates',
  async(cityName: string, thunkAPI) => {
    try {
      const coordinates = await axios.get(`${geocodingApiUrl}/direct?q=${cityName}&limit=1&appid=${apiKey}`, {});
      if (coordinates.data.length === 0) {
        return thunkAPI.rejectWithValue('City not found');
      }
      const {lat, lon, name} = coordinates.data[0];
      const weatherResponse = await axios.get(`${apiUrl}?lat=${lat}&lon=${lon}&units=metric&appid=${apiKey}`, {});
...
export const updateWeather = createAsyncThunk('updateWeatherData',
